...
export async function POST(req: Request) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: true, status: data?.status || 'pending' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    })
  } catch (error) {
    console.error('Twilio send OTP error:', error)
    return new Response(JSON.stringify({ error: (error as Error).message || 'Failed to send OTP.' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    })
  }
